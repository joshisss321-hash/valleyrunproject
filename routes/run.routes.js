...
const Registration  = require("../models/Registration");
const User          = require("../models/User");
const EventModel    = require("../models/Event"); // ✅ Added
...
router.post("/search-runner", async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({ runner: null, message: "No registration found for this event" });
    }
...
router.post("/submit-run", upload.single("image"), async (req, res) => {
  try {
    ...
    // ✅ Registration deadline check — Yoga Day jaise events block honge
    const eventDoc = await EventModel.findOne({ slug: eventSlug });
    if (eventDoc) {
      const now = new Date();
      const regDeadline = eventDoc.registrationDeadline
        ? new Date(eventDoc.registrationDeadline)
        : null;
      const regStillOpen = eventDoc.isRegistrationOpen === true &&
        regDeadline && regDeadline > now;

      if (regStillOpen) {
        cleanup(req.file);
        return res.status(400).json({
          error: "Submissions will open after registration closes. Please wait!",
        });
      }
    }
    ...
    // ✅ Upload to ImageKit with compression
    const sharp = require("sharp");
    const fileBuffer = await sharp(req.file.path)
      .resize(1200, 1200, { fit: "inside", withoutEnlargement: true })
      .jpeg({ quality: 70 })
